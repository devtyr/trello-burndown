...
CardStatistics.prototype.generate = function(cards, finishLists, standuptime, callback) {
	...
	var reg = /^\[(\d+)\|(\d+(?:\.\d+)?)-(\d+(?:\.\d+)?)\]\s*(.*)$/;
	var reg_trelloscrum_withEffort = /^\[(\d+(?:\.\d+)?)\]\((\d+(?:\.\d+)?)\)\s*(.*)$/;
	var reg_trelloscrum_withoutEffort = /^\((\d+(?:\.\d+)?)\)\s*(.*)$/;
	...
	for (var i = 0; i < cards.length; i++) {
		...
		var isTrelloScrumWithoutEffort = false;
		var matches = reg.exec(title);

		if (!matches) {
			matches = reg_trelloscrum_withEffort.exec(title);
			if(!matches){
				matches = reg_trelloscrum_withoutEffort.exec(title);
				if(matches)
					isTrelloScrumWithoutEffort = true;
			}
