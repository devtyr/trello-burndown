...
CardStatistics.prototype.generate = function(cards, finishLists, standuptime, callback) {
    ...
    var reg_agilescrum_for_trello = /^(?=[^(]*\(\s*(\d+(?:\.\d+)?)\s*(\/(\d+(?:\.\d+)?))?\s*\))(.*)$/; 
    // (1.0/2.0), that is (effort/estimate)
    //     or
    // (2.0), that is (estimate)
	...
	for (var i = 0; i < cards.length; i++) {
		...
		var isAgileScrumForTrelloMatch = false;
		var matches = reg.exec(title);

		if(!matches) {
			matches = reg_agilescrum_for_trello.exec(title);
			if(matches) {
				isAgileScrumForTrelloMatch = true;
			}
		}
		...
		if (matches && matches.length > 1) {
			...
			} else if (isAgileScrumForTrelloMatch) {
				if (matches[3]) {
					estimate = parseFloat(matches[3]);
					effort = parseFloat(matches[1]);
				} else {
					estimate = parseFloat(matches[1]);
				}
