...
Helper.prototype.generateAndExportFromConfig = function(name, generateAndExportFromConfig_callback) {
	...
	if (fs.existsSync(configPath)) {
		...
		if (!this.isSprintActive(name)) {
			...
			this.generateAndExport(configuration.lists, configuration.days, configuration.resources, configuration.finishedList, configuration.standupTime, configuration.name, generateAndExportFromConfig_callback);
		}
	} else {
...
Helper.prototype.generateAndExport = function(lists, days, resources, finishedLists, standuptime, name, callback) {
	var receiver = new CardReceiver(settings.applicationKey, settings.userToken, settings.boardId);
	var splittedLists = lists.split(',');
	splittedLists.push(finishedLists);
