...
main.use('/', function(req, res, next) {
	...
	var html = getTemplate().replace('{{title}}','trello burndown chart generator').replace('{{header}}', '<h1>Burndown for sprint "' + sprint + '"</h1>').replace('{{script}}', dataScript).replace('{{effortScript}}', effortScript);
...
function getTemplate() {
	return  '<html>' +
			...
			'<figure style="width: 800px; height: 250px;" id="burndownChart"></figure>' +
			'<p>' +
			'<span>dotted: ideal estimate</span></br>' +
			'<span>line: done estimate</span>' +
			'<h2>Done estimate vs. effort (daily)</h2>' +
			'<figure style="width: 800px; height: 250px;" id="effortChart"></figure>' +
			'<p>' +
...
function generateDataScript(sprintData) {
    ...
    for (var line = 0; line < sprintData.length; line++)
    {
    	...
    	retVal += template.replace("{{x}}", parseInt(lineData.day)).replace("{{y}}", parseInt(lineData.openEstimate));
	...
	for (var line = 0; line < sprintData.length; line++) {
		var lineData = sprintData[line];
		retVal += template.replace("{{x}}", parseInt(lineData.day)).replace("{{y}}", parseInt(lineData.idealEstimate));
    	if (line + 1 < sprintData.length)
    		retVal += ",";	
	}
...
function generateEffortScript(sprintData) {
    ...
    {
    	var lineData = sprintData[line];
    	retVal += template.replace("{{x}}", parseInt(lineData.day)).replace("{{y}}", parseInt(lineData.doneEstimate));
    	if (line + 1 < sprintData.length)
    		retVal += ",";
    }
	...
	for (var line = 0; line < sprintData.length; line++) {
		var lineData = sprintData[line];
		retVal += template.replace("{{x}}", parseInt(lineData.day)).replace("{{y}}", parseInt(lineData.effort));
    	if (line + 1 < sprintData.length)
    		retVal += ",";	
	}
